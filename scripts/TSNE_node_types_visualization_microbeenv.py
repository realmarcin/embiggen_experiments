...
# embedding_path = "/global/homes/m/marcinj/data/ENIGMA/SkipGram_embedding.npy"
...
#LOAD EXISTING tSNE embeddings
# os.makedirs("tsne", exist_ok=True)
#for perplexity in tqdm(perplexities, desc="Perplexities", leave=False):
#    tsne_path = f"tsne_microbeenv/microbeenv_node_{perplexity}"
...
for perplexity, axes in zip(perplexities, axes_matrix):
    tsne_embedding = np.load(f"tsne_microbeenv/microbeenv_node_{perplexity}.npy")
    plot_embedding(
        load_graph(node_types_columns),
        tsne_embedding,
        10,
        axes=axes[0]
    )
    axes[0].set_title(f"Perp. {perplexity}")
###SAVE
fig.savefig('microbeenv_node_types.png')
# plt.show()


graph = load_graph(node_types_columns)
...
###SAVE
fig.savefig('microbeenv_node_degrees.png')
# plt.show()
